function [FRA_cell , FRA_signif_cell , BF_mat, lat_to_BFfirstspike ] = find_BF_in_best_resp_wind_3( data_table ,...
          PRE_STIM_MSEC , protocol_struct , window_size , check_excitation )

    % The function FIND_BF_IN_BEST_RESP_WIND_3 is a function that finds the
    % best frequency for excited units. The function receives the data
    % table, the protocol_struct, the size of the excitatory window and the
    % pre stimulus baseline time (PRE_STIM_MSEC). The parameter
    % "check_excitation" validates is set to 1 if we want to check whether
    % the unit is significantly excitated or 0 if we don't (and we 
    % calculate the FRA anyway).
    ...
    n_reps = protocol_struct.n_reps ;                                       % repetitions of stimulus
    n_attens = length( protocol_struct.atten ) ;                            % number of attenuations
